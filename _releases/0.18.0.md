...
- `analyzepsbt` examines a PSBT and prints the next step the user needs
  to take towards finalizing it.
...
- `utxoupdatepsbt` searches the set of Unspent Transaction Outputs
  ...
  segwit requires the signing of information that is not directly
  contained in the spending transaction or derivable from the signer's
  private key, such as the value of the output being spent.
...
- Be reminded that parts of the `validateaddress` command have been
  ...
  `script`, `hex`, `pubkeys`, `sigsrequired`, `pubkey`, `addresses`,
  `embedded`, `iscompressed`, `label`, `timestamp`, `hdkeypath`,
  `kdmasterkeyid`.
...
- #14897 Randomize GETDATA(tx) request order and introduce bias toward outbound (naumenkogs)
